...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return [ torch.tensor(p[idx]) for p in self.params ]
...
def main(argv):
    ...
    tf_x_train_miss = DataLoader(
        MyDataset(x_train_miss, m_train_miss),
        shuffle=True,
        batch_size=FLAGS.batch_size)
    tf_x_val_miss = iter(DataLoader(
        MyDataset(x_val_miss, m_val_miss), batch_size=FLAGS.batch_size, shuffle=False))
    tf_x_test_miss = DataLoader(MyDataset(x_val_miss, m_val_miss), batch_size=len(x_val_miss), shuffle=False)
    ...
    for i, (x_seq, m_seq) in enumerate(tf_x_train_miss):
        ...
        try:
            ...
            if i % FLAGS.print_interval == 0:
                    print("================================================")
                    print("Learning rate: {} | Global gradient norm: ".format(optimizer.param_groups[0]['lr']))
                    ...
                    if FLAGS.data_type in ["hmnist", "sprites"]:
                        ...
                        tf.contrib.summary.image("input_train", x_seq.reshape([-1]+list(img_shape)))
                        tf.contrib.summary.image("reconstruction_train", x_hat.reshape([-1]+list(img_shape)))
                    elif FLAGS.data_type == 'physionet':
                        ...
                        mse_miss = np.sum([model.compute_mse(x, y=y, m_mask=m).item()
                                           for x, y, m in get_val_batches()]) / n_missings

                        x_val_imputed = np.vstack([
                            model.decode(model.encode(x_batch).mean).mean.detach().numpy()
    ...
    nll_miss = np.sum([model.compute_nll(x, y=y, m_mask=m).item()  #.detach().numpy()
                       for x, y, m in get_val_batches()]) / n_missings
    mse_miss = np.sum([model.compute_mse(x, y=y, m_mask=m, binary=FLAGS.data_type=="hmnist").item() #.detach().numpy()
                       for x, y, m in get_val_batches()]) / n_missings
    print("NLL miss: {:.4f}".format(nll_miss))
    print("MSE miss: {:.4f}".format(mse_miss))

    # Save imputed values
    z_mean = [model.encode(x_batch).mean.detach().numpy() for x_batch in x_val_miss_batches]
    np.save(os.path.join(outdir, "z_mean"), np.vstack(z_mean))
    x_val_imputed = np.vstack([model.decode(z_batch).mean.detach().numpy() for z_batch in z_mean])
